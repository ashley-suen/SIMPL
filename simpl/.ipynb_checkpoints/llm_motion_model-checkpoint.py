...
class SmolLMMotionPredictor(nn.Module):
    def __init__(self,
        ...
        self.mlp_head = nn.Sequential(
            ...
            nn.Linear(512, 256),
            nn.GELU(),
            nn.Linear(256, num_future_steps * 2),
        )

        # Move entire model to target device and ensure dtype consistency.
        # All modules (LLM, social_attn, mlp_head) must be on the same device
        # and use the same dtype for DDP compatibility.
        if device is not None:
            self.to(device)
            # Convert social_attn and mlp_head to match LLM dtype
    ...
    def forward(self, input_ids, attention_mask, agent_valid):
        """
        Plan A: each agent has its own independent token sequence.
        The LLM is shared across all agents (weight-sharing).
        Social context is added via cross-attention after individual encoding.

        Args:
            input_ids:      [B, N, L]  — per-agent token sequences
            attention_mask: [B, N, L]  — 1 for real tokens, 0 for padding
            agent_valid:    [B, N]     — bool, True if agent exists in scene
        Returns:
            predicted_trajs: [B, N, 60, 2]
        """
        B, N, L = input_ids.shape

        # Batch all N agents together for a single efficient LLM call
        ids_flat  = input_ids.view(B * N, L)       # [B*N, L]
        mask_flat = attention_mask.view(B * N, L)   # [B*N, L]
        ...
        # Mean-pool over non-padding tokens to get one vector per agent
        mask_exp = mask_flat.unsqueeze(-1).float()                          # [B*N, L, 1]
        pooled   = (hidden * mask_exp).sum(dim=1) / \
                   mask_exp.sum(dim=1).clamp(min=1e-9)                      # [B*N, H]

        agent_repr = pooled.view(B, N, H)                                   # [B, N, H]

        # Zero out invalid (dummy) agent slots before social attention
        agent_repr = agent_repr * agent_valid.unsqueeze(-1).float()         # [B, N, H]

        # Social cross-attention: each agent attends to all other valid agents
        agent_repr = self.social_attn(agent_repr, agent_valid)              # [B, N, H]

        # MLP head: predict future displacements for every agent
        pred_flat       = self.mlp_head(agent_repr.view(B * N, H))         # [B*N, 60*2]
        predicted_trajs = pred_flat.view(B, N, self.num_future_steps, 2)   # [B, N, 60, 2]
...
if __name__ == "__main__":
    ...
    dummy_attn_mask[0, -1, 10:] = 0   # simulate a short last-agent sequence
    dummy_agent_valid = torch.ones(B, N_agents, dtype=torch.bool)
    dummy_agent_valid[0, -1] = False   # last agent slot empty in first sample

    print("Initializing Model (Plan A — separate agent encoding)...")
    ...
    assert out.shape == (B, N_agents, 60, 2), "Output geometry is incorrect."
